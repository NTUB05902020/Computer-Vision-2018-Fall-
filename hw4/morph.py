...
class PIC:
	...
	def dilation(self, pic):
		...
		tmp = set([(r1+r2,c1+c2) for r1,c1 in self.Ws for r2,c2 in pic.Ws])
		return PIC(self.row, self.col, whole.intersection(tmp))
	def erosion(self, pic):
		...
		inBound = set([(i,j) for i in range(mr1-mr2, Mr1-Mr2+1) for j in range(mc1-mc2, Mc1-Mc2+1)])
		new.Ws = inBound.intersection(new.Ws)
		return new
	...
	def complement(self):
		if self.Bs == None:
			whole = set([(i,j) for i in range(self.row) for j in range(self.col)])
			self.Bs = whole.difference(self.Ws)
...
tffft = PIC(5, 5, [(-2,-1),(-2,0),(-2,1),(-1,-2),(-1,-1),(-1,0),(-1,1),(-1,2),\
